...
class GeneralHeap:
    def __init__(self, root = None, isMax=False) -> None: # This class is to show that you can use a maxHeap and a minHeap in the same class.
        ...
        if isMax:
            self.compare = compareGT
        else:
            self.compare = compareLT
    ...
    def insert(self, data):
        ...
        while self.compare(self.heap[currPos],self.heap[self.parent(currPos)]):
    ...
    def heapifyPos(self, pos):
        if not self.isLeaf(pos):
            ...
            if leftPos < len(self) and self.compare(self.heap[leftPos], self.heap[pos]):
                self.swap(pos, leftPos)
            elif rightPos < len(self) and self.compare(self.heap[rightPos], self.heap[pos]):
